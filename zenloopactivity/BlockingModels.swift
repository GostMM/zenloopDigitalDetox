...
//
//  Modèles pour le système de blocage d'apps (copie identique de l'app principale)
//
...
private let blockLogger = Logger(subsystem: "com.app.zenloop.zenloopactivity", category: "BlockManager")
...
struct BlockManager {
    ...
    func getAllBlocks() -> [ActiveBlock] {
            ...
            if let containerURL = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.com.app.zenloop") {
                ...
                if FileManager.default.fileExists(atPath: fileURL.path) {
                    do {
                        ...
                        blockLogger.info("✅ [BlockManager] Data loaded from file: \(data!.count) bytes")
                    } catch {
                        blockLogger.error("❌ [BlockManager] File read error: \(error.localizedDescription)")
                    }
                }
            }
        ...
        do {
            ...
            blockLogger.info("✅ [BlockManager] \(blocks.count) blocks loaded")
            return blocks
        } catch {
            blockLogger.error("❌ [BlockManager] Decode error: \(error.localizedDescription)")
            return []
        }
    }
    ...
    func addBlock(appName: String, duration: TimeInterval, tokenData: Data? = nil, context: String = "") -> ActiveBlock {
        ...
        blockLogger.info("➕ [BlockManager] Adding: \(appName) for \(Int(duration/60))min")

        saveBlock(block)
        blockLogger.info("✅ [BlockManager] Block saved: \(block.id)")
